...
export default function FormularioReservacion({
  ...
  const selectedDate = formFecha ? new Date(formFecha) : null;
...
<div className="bg-zinc-800 rounded-xl p-6 shadow-xl w-full">
  ...
  <div className="grid grid-cols-7 gap-1 text-center">
    ...
    {daysInCurrentMonth.map((day) => {
      // Nueva función para comparar la fecha con el formFecha en formato YYYY-MM-DD
      const isSelected = (day) => {
        const date = new Date(currentYear, currentMonth, day);
        const localISO = date.toLocaleDateString("sv-SE"); // formato YYYY-MM-DD
        return formFecha === localISO;
      };
...
<div className="bg-zinc-800 rounded-xl shadow-xl p-6 w-full">
  ...
  <div className="grid grid-cols-2 xl:grid-cols-3 gap-4 max-h-80 overflow-y-auto scrollbar-thin">
    {servicios.map((s) => (
      <button
        key={s.id}
        onClick={() => setFormServicioId(s.id.toString())}
...
<div className="bg-zinc-800 rounded-xl shadow-xl p-6 w-full ">
  ...
  <div
    className="grid grid-cols-2 gap-4 max-h-72 overflow-y-auto pr-2"
...
{/* Empleados con scroll horizontal */}
<div className="bg-zinc-800 rounded-xl shadow-xl p-6 w-full">
  ...
  <div className="overflow-x-auto scrollbar-thin scrollbar-thumb-zinc-700 scrollbar-track-zinc-800">
    <div className="flex gap-x-4 pb-2">
      {empleados.map((e) => (
        <button
          key={e.id}
          onClick={() => setFormEmpleadoId(e.id.toString())}
        ...
        >
          <div className="w-14 h-14 mx-auto rounded-full bg-zinc-700 mb-2"> <img className="rounded-full" src={e.foto_perfil} alt="" /></div>
