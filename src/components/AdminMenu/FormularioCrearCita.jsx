...
export default function FormularioCrearCita({ servicios, empleados, horarios, onCrear }) {
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-r from-zinc-900 via-black to-zinc-900 text-white py-8 px-4">
        ...
        <div className="space-y-6 w-full">
          <div className="bg-zinc-800 rounded-xl p-6 shadow-xl w-full">
            ...
            <div className="grid grid-cols-7 gap-1 text-center">
              {Array((firstDayOfMonth + 6) % 7).fill(null).map((_, i) => <div key={"empty-" + i}></div>)}
              {daysInCurrentMonth.map((day) => {
                const date = new Date(currentYear, currentMonth, day);
                const localISO = date.toLocaleDateString("sv-SE");
                const selected = fecha === localISO;
        ...
        <div className="space-y-6 w-full">
          <div className="bg-zinc-800 rounded-xl shadow-xl p-6 w-full">
            ...
            <div className="grid grid-cols-2 gap-4 max-h-72 overflow-y-auto pr-2" style={{ scrollbarWidth: "thin" }}>
              {horarios.map((h, idx) => {
                const selected = hora === h.inicio;
                return (
                  <button
                    key={idx}
                    onClick={() => setHora(h.inicio)}
                    className={`px-6 py-3 rounded-lg font-semibold text-base transition duration-300 ease-in-out border border-transparent shadow-sm
                      ${selected ? "bg-yellow-400 text-black shadow-lg border-yellow-300" : "bg-white text-zinc-800 hover:bg-yellow-100 hover:shadow-md"}
                      hover:-translate-y-0.5 active:translate-y-0 focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:ring-opacity-50`}
                  >
                    {h.inicio} - {h.fin}
                  </button>
                );
            ...
            <div className="overflow-x-auto scrollbar-thin scrollbar-thumb-zinc-700 scrollbar-track-zinc-800">
              <div className="flex gap-x-4 pb-2">
                {empleados.map((e) => (
                  <button
                    key={e.id}
                    onClick={() => setEmpleadoId(e.id.toString())}
      ...
      {errores.mensaje && (
        <div className="mt-6 bg-red-100 border border-red-400 text-red-700 rounded-md px-4 py-3 font-medium text-base max-w-3xl mx-auto">
          {errores.mensaje}
        </div>
      )}
    </div>
